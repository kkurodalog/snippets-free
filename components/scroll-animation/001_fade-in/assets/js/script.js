 ...
 *   対象を増やせる。
 * - このパーツが受け持つのは「1つの要素をふわっと出す」ことだけである。
 *   複数の要素を時間差で順に出す用途は、順次表示のパーツが受け持つ。
 *   そのため遅延の受け付けはこのパーツには無い（下記「属性」を参照）。
 ...
 * - data-fade-in-direction 出現方向（up / down / left / right / none）。省略時は up。
 *                       none は transform を使わず opacity だけの純フェード。
 *   遅延を指定する属性は持たない。要素は交差した時点でそのまま出る。
 *   時間差を作りたい場合は、順次表示のパーツ（親に印を付けると子の遅延を
 *   連番で配る形）を使うこと。
 *
 * 観測オプション:
